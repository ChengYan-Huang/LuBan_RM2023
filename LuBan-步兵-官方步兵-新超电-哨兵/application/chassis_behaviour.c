...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    //计算遥控器的原始输入信号
		...
		//小陀螺：要做的就是把旋转角度算出来
		if (chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY || chassis_move_rc_to_vector->chassis_RC->rc.ch[4]>150)
		{
		...
		else if(chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY || chassis_move_rc_to_vector->chassis_RC->rc.ch[4]<-150)
		{
		revolve_add=0.0f;
		revolve_flag=0;
