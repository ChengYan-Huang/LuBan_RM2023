...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    //计算遥控器的原始输入信号
		...
		//小陀螺：要做的就是把旋转角度算出来
		if (chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY || chassis_move_rc_to_vector->chassis_RC->rc.ch[4]>150||get_beida()!=0)
		{
				revolve_add=5.0f;//0.08
			
				revolve_flag=1;
			
			
			
//			if(chassis_voltage<18.f&&chassis_state==0)
//			{
//			revolve_add=3.0f;//0.08
//    		revolve_flag=1;
//			}
			
			revolve_angle=revolve_add;
//			if(chassis_move_rc_to_vector->chassis_RC->rc.ch[4]<-150||get_game_state()==5)
//			{
//				revolve_add=0.0f;
//				revolve_flag=0;
//			
...
//			}
		...
		else if(chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY || chassis_move_rc_to_vector->chassis_RC->rc.ch[4]<-150||get_game_state()==5)
		{
		revolve_add=0.0f;
		revolve_flag=0;
	...
	if(revolve_flag){*wz_set = revolve_angle;}
	else{
